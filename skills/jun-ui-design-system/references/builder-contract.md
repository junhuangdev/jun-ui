...
`bundle-app` injects Semi's `--semi-*` token surface plus the small `--jun-ui-*` delivery-variable layer into the bundled CSS. Target project CSS should reference Semi visual tokens and `jun-ui` delivery variables directly; it should not define local visual aliases or new page color literals.
