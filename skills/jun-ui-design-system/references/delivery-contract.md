...
- **Emphasis devices.** Express "this matters / needs action" through Semi-native means: elevation (shadow), a tinted surface, the Semi `Tag`, or section position and order. Do not hand-roll a colored **accent rail** — a single-side `border-left` / `-right` / `-top` / `-bottom` painted in an emphasis color (primary / brand / semantic) and used as a spine — nor hand-rolled accent dots or bars. A colored spine is foreign to Semi's surface language and tends to be applied to some cards but not others, so it reads as uncoordinated; it also slips past the bare-color gate when the color is a token (e.g. `border-left: var(--jun-ui-action-border-width) solid var(--semi-color-primary)`). Neutral single-side borders — dividers in `--semi-color-border` / fill — are fine. `verify-page --strict` surfaces token-colored accent rails in source CSS as advisories. (advisory)
...
- **No project-local visual tokens.** Target project CSS must not define local visual custom properties such as `--ck-*`, `--prod-*`, `--ff-*`, `--panel-bg`, `--surface`, or `--font-size-meta`, even when their values point at Semi tokens. Consume Semi `--semi-*` visual tokens and `--jun-ui-*` delivery/layout variables directly. `verify-page --strict` rejects project-local visual custom property definitions in source CSS. (hard gate)
...
- **Do not fork the system.** Page source CSS must not redefine Semi `--semi-*` tokens, `--jun-ui-*` delivery variables, `.jui-stack` / `.jui-row` utilities, or project-local visual token aliases. Consume Semi and jun-ui variables directly inside the declarations that need them. (hard gate)
