...
- **Radius / geometry.** Comes from the theme bridge (`--jun-ui-radius`). Do not hardcode corner radii to re-create Semi defaults; `verify-page --strict` surfaces literal radii in source CSS as advisories (`var(...)` aliases, `0`, `50%` circles, and `999px` pills pass). (advisory)
