...
- **Scroll regions.** A height-bounded content region must scroll, not clip: use `jui-scroll-y` (it bundles `overflow-y:auto`, `min-height:0`, and a themed scrollbar). Never leave `overflow:hidden` on a height-limited content area — it clips silently with no scrollbar.
