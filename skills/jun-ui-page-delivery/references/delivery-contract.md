...
- **Scroll end breathing room.** Content scrolled to the bottom must never sit flush against the container edge. `jui-scroll-y` bundles this; any scroller that bypasses the utility — especially Semi internal scroll bodies (SideSheet, Modal, Drawer, Table body) — must add explicit end padding (`padding-bottom: var(--jun-ui-section-gap)` on your innermost content wrapper). Recurring real-world defect; treat any flush-bottom scroll region as a review blocker. (hard gate)
